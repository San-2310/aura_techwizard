...
class _DietPlanScreenState extends State<DietPlanScreen> {
  ...
  final List<String> _healthConditionOptions = [
    ...
    'Insomnia', 'Digestive Issues', 'None'
  ];
    ...
    try {
      gemini.streamGenerateContent(prompt).listen(
        ...
        onError: (error) {
          setState(() {
            _isLoading = false;
            _generatedPlan = 'Error generating plan. Please try again. Error: $error';
          });
        },
      );
    } catch (e) {
...
String _formatStreamResponse(String response) {
  ...
  cleanText = cleanText
    ...
    .replaceAll(RegExp(r'\*\*Meal Prep Ideas.*?:\*\*'), '\nMeal Prep Ideas:\n')
...
List<TextSpan> _formatTextWithHeaders(String text) {
  ...
  String cleanText = response.replaceAll(RegExp(r'Candidates\(.*?text: '), '')
      .replaceAll(RegExp(r'\), finishReason:.*?NEGLIGIBLE\]\)'), '')
      .replaceAll('Parts(text: ', '')
      .replaceAll('), role: model', '');
